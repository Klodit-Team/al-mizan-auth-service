...
export async function consumeUserRegisteredResponse(): Promise<void> {
  if (!channel) {
    throw new Error('RabbitMQ channel not initialized');
  }
  ...
  await channel.consume(USER_REGISTERED_RESPONSE_QUEUE, async (msg) => {
    ...
    try {
      const payload = JSON.parse(msg.content.toString()) as UserRegisteredResponsePayload;

      if (payload.status === 'success') {
        console.log(
          `[USER.REGISTERED.RESPONSE] ✓ success → user_id=${payload.user_id}, profile_id=${payload.profile_id}`
        );

        const notificationPayload: NotificationUserPayload = {
        ...
        console.log(
          `[NOTIFICATIONS.USER] queued → user_id=${payload.user_id}, email=${payload.email}, langue=${notificationPayload.langue}`
        );
      } else {
        console.error(
          `[USER.REGISTERED.RESPONSE] ✗ failed → user_id=${payload.user_id}, reason=${payload.reason ?? 'Unknown reason'}`
        );
      }
...
export async function consumeUserRegisteredFailed(): Promise<void> {
  if (!channel) {
    throw new Error('RabbitMQ channel not initialized');
  }
  ...
  await channel.consume(USER_REGISTERED_FAILED_QUEUE, async (msg) => {
    ...
    try {
      const payload = JSON.parse(msg.content.toString()) as UserRegisteredResponsePayload;
      console.error(
        `[USER.REGISTERED.FAILED] ✗ user_id=${payload.user_id}, reason=${payload.reason ?? 'Unknown reason'}`
      );
      channel.ack(msg);
    } catch (error) {
