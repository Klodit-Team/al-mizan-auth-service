...
const usersServiceUrl = (path) => {
    const normalizedPath = path.startsWith('/') ? path : `/${path}`;
    return `${USERS_SERVICE_BASE_URL}${USERS_SERVICE_PREFIX}${normalizedPath}`;
};
const getUsersServiceErrorMessage = (payload, fallback) => {
...
const usersServiceRequest = async (method, path, body) => {
    ...
    if (!response.ok) {
        ...
        throw new UsersServiceRequestError(response.status, getUsersServiceErrorMessage(payload, fallbackMessage));
    }
    return payload;
};
const resolveFallbackLanguage = (langue) => {
    if (langue === 'ar') {
        return 'ar';
    }
    return 'fr';
};
const resolveFallbackOrganisationType = (type) => {
    if (typeof type !== 'string' || !type.trim()) {
        return 'ENTREPRISE_PRIVEE';
    }
    return type.trim().toUpperCase();
};
const getRoleIdForFallback = async (roleName) => {
    const findRole = (roles) => roles.find((role) => role.name === roleName);
    let roles = await usersServiceRequest('GET', '/roles');
    let targetRole = findRole(roles);
    if (!targetRole) {
        await usersServiceRequest('POST', '/roles/seed-defaults');
        roles = await usersServiceRequest('GET', '/roles');
...
const normalizeAuthRoleName = (value) => {
    ...
    if (normalized === 'MEMBRE_COMMISSION')
        return 'MEMBRE_COMMISSION';
    if (normalized === 'CONTROLEUR')
        return 'CONTROLEUR';
    return null;
};
const mapRoleToDashboardUserType = (role) => {
    if (!role) {
        return null;
    }
    if (role === 'ADMIN') {
...
const resolvePrimaryUserRole = async (userId) => {
    try {
        const assignments = await usersServiceRequest('GET', `/user-roles/${userId}`);
...
const provisionRegistrationFallback = async (event, canonicalRole) => {
    const organisation = await usersServiceRequest('POST', '/organisations', {
...
const validateRegisterPayload = (payload, canonicalRole) => {
    ...
    if (!SUPPORTED_ORGANISATION_TYPES.has(payload.type.trim().toUpperCase())) {
...
export const sessions = async (req, res) => {
    const active = await tokenService.getActiveSessions(req.user.userId);
    res.json({ sessions: active });
};
export const deleteSession = async (req, res) => {
    const sessionId = req.params['id'];
    await tokenService.revokeSession(sessionId, req.user.userId);
